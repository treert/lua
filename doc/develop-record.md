## 2025-7-17 mylua 调用函数时支持 *args 传参
兴致来了，给 mylua 增加上 *args 传参支持。
不过对 mylua 热情也减退了，vscode 插件停止开发好久了。现在用python 多了起来。

一些零碎
1. 现在AI编程非常厉害，编程语言需要支持让ai输出各种常见解决方案代码，python就是如此。
   - mylua 要想真正当成个人主力脚本，需要积累一批稳定的库，并给ai训练处一批解决方案，才会好用。
2. 分代gc存在 MajorGC STW 的问题，个人决定不适合lua，甚至也不适合 dart。
3. 发现 dart 语言，算是加强版的lua。不过也有不喜的地方
   - 不能干净，编译还依赖google的工具。
   - 不支持 RAII，lua 都支持了 close 变量了，它不支持。
